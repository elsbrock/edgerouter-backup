...
then
  ...
  curl -Ls https://github.com/elsbrock/go-ghwrite/releases/latest/download/go-ghwrite_linux-$(lscpu | grep -i arch | awk '{print $2}').tar.gz  | tar xzvf - -C $(dirname $GOGHWRITE_BIN)
fi
